...
from task_queue import celery_app
from scrapers.scrapers.spiders.amazon import run_spider2
from scrapers.scrapers.spiders.flipkart import run_flipkart_billiard_spider
...
@celery_app.task(name='watchcord.celery_tasks.tasks.scrape_flipkart_task')
def scrape_flipkart_task(urls: str):
    url_list = urls.split(',')
    data = run_flipkart_billiard_spider(url_list)
    print(f"Data: {data}")
    return data
...
@celery_app.task(name='watchcord.celery_tasks.tasks.scrape_flipkart_with_webhook_task')
def scrape_flipkart_with_webhook_task(urls: str, user: str):
    webhook = SyncWebhook.from_url("https://discord.com/api/webhooks/1295990950216335400/X-8Y0HsMygXza-0MGBraAe15OA6msYj6oiWoW_2_GXAQCwAM429Lt_NUq--bpECIgkUe")
    url_list = urls.split(',')
    data = run_flipkart_billiard_spider(url_list)
    print(f"Data: {data}")
    print(f"Sending Data via Webhook: {webhook.url}")
    for i in range(len(data['fsns'])):

        tracked_product_db = requests.post("http://localhost:8000/createtrackedproduct/", json=TrackedProduct(product_id=data['fsns'][i], title=data['titles'][i], mrp=data['mrps'][i], discount_percentage=data['discount_percentages'][i], current_price=data['current_prices'][i], categories=data['categories'][i], description=data['descriptions'][i], rating=data['ratings'][i], domain=data['domains'][i], image=data['images'][i]).model_dump())
        print("Tracked Product DB Response: ",tracked_product_db.text)
        if tracked_product_db.status_code == 409:
            tracked_product_db_update = requests.put(f"http://localhost:8000/updatetrackedproduct/{data['domains'][i]}/{data['fsns'][i]}", json=TrackedProduct(product_id=data['fsns'][i], title=data['titles'][i], mrp=data['mrps'][i], discount_percentage=data['discount_percentages'][i], current_price=data['current_prices'][i], categories=data['categories'][i], description=data['descriptions'][i], rating=data['ratings'][i], domain=data['domains'][i], image=data['images'][i]).model_dump())
            print("Tracked Product DB Response: ",tracked_product_db_update.text)
        ...
        embed.set_author(name=f"{user}'s Scraped Product")
        embed.add_field(name="Domain", value=data['domains'][i], inline=True)
        embed.add_field(name="Product ID", value=data['fsns'][i], inline=True)
        embed.add_field(name="Rating", value=data['ratings'][i], inline=True)
        embed.add_field(name="MRP", value=data['mrps'][i], inline=True)
        embed.add_field(name="Discount", value=data['discount_percentages'][i], inline=True)
        embed.add_field(name="Current Price", value=data['current_prices'][i], inline=True)
...
def schedule_flipkart_tasks_with_redbeat(urls, schedule_for, user):
    interval = celery.schedules.schedule(run_every=schedule_for)
    entry = RedBeatSchedulerEntry(name="scrape_flipkart_with_webhook_task"+str(uuid4()), task="watchcord.celery_tasks.tasks.scrape_flipkart_with_webhook_task", schedule=interval, args=[urls, user], app=celery_app)
    entry.save()
    return {"status":"Task Scheduled", "task_key":entry.key}

def remove_flipkart_scheduled_tasks_with_redbeat(entry_key):
    entry = RedBeatSchedulerEntry.from_key(entry_key, app=celery_app)
    entry.delete()
    return {"status":f"Task {entry_key} Deleted"}